...
try {
    ...
    if (!$current) throw new Exception("Plan not found");
    ...
    // --- Prepare updateData ---
    $fields = ['plan_name','user_id','group_user_id','company_id','frequency_number','frequency_unit',
               'frequency_type','start_waranty','start_date','end_date','cost_type','price','type_ma','contract','is_active'];
    $updateData = [];
    foreach ($fields as $f) {
        $updateData[$f] = $input[$f] ?? $current[$f];
        if (in_array($f,['start_waranty','start_date','end_date']) && $updateData[$f]==='') $updateData[$f]=null;
    }
    ...
    if ($hasResult) {
        ...
        foreach ($restrictedFields as $rf) {
            if (isset($input[$rf]) && $input[$rf] !== $current[$rf]) $restrictedFieldsRetained = true;
    ...
    if (isset($input['is_active']) && count($input)===2) {
        ...
        if ($changed) {
            $log->insertLog($user_id, 'maintenance_plans', 'UPDATE',
    ...
    if (!in_array($updateData['type_ma'],$allowed_type_ma)) throw new Exception("Invalid type_ma");
    if (!in_array($updateData['cost_type'],$allowed_cost_type)) throw new Exception("Invalid cost_type");
    if (!in_array((int)$updateData['frequency_unit'],$allowed_frequency_unit)) throw new Exception("Invalid frequency_unit");

    // --- คำนวณรอบ ---
    $roundDates=[]; $intervalCount=0;
    if (!$hasResult) {
        if ($updateData['frequency_type']==='รอบเดียว') {
            ...
            while($temp<=$end){
                ...
                switch($intervalUnit){
                    case 1: $temp->add(new DateInterval('P'.$intervalNumber.'D')); break;
                    case 2: $temp->add(new DateInterval('P'.($intervalNumber*7).'D')); break;
                    case 3: $temp->add(new DateInterval('P'.$intervalNumber.'M')); break;
                    case 4: $temp->add(new DateInterval('P'.$intervalNumber.'Y')); break;
                }
            }
        }
    } else $intervalCount=$current['interval_count'];
    ...
    if ($changedFields){
        $oldValues=[];
        foreach($changedFields as $f=>$v) $oldValues[$f]=$current[$f];
        $log->insertLog($user_id,'maintenance_plans','UPDATE',
    ...
    if (!$hasResult){
        ...
        if($roundDates !== $oldRounds){
            ...
            foreach($roundDates as $rd) $stmtIns->execute([':plan_id'=>$input['plan_id'], ':start_date'=>$rd]);

            $log->insertLog($user_id,'details_maintenance_plans','UPDATE',
