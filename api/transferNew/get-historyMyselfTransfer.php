...
try {
    ...
    // ดึง department_id ของผู้ใช้ก่อน
    $userStmt = $dbh->prepare("SELECT department_id FROM users WHERE ID = :u_id");
    $userStmt->execute([':u_id' => $u_id]);
    $userDept = $userStmt->fetchColumn();
    if (!$userDept) throw new Exception("User department not found");

    $searchCondition = '';
    $filterCondition = '';
    $params = [':u_id' => $u_id, ':userDept' => $userDept];
    ...
    if (!empty($search)) {
        $searchCondition = "AND (
            ...
            d_now.department_name LIKE :search OR
    ...
    $baseWhere = "
        WHERE (
            ht.to_department_id = :userDept
            OR ht.from_department_id = :userDept
            OR ht.now_subcategory_id IN (
                SELECT rg.subcategory_id
                FROM relation_group rg
                JOIN group_user gu ON rg.group_user_id = gu.group_user_id
                JOIN relation_user ru ON ru.group_user_id = gu.group_user_id
                WHERE ru.u_id = :u_id
    ...
    //  นับจำนวนทั้งหมด
    $countSql = "
        SELECT COUNT(DISTINCT ht.history_transfer_id)
        ...
        LEFT JOIN departments d_now ON ht.now_equip_location_department_id = d_now.department_id
        {$baseWhere}
    ";
    ...
    $sql = "
        SELECT DISTINCT
            ...
            d_now.department_name AS now_location_name,
            CASE
    ...
    ";
    if ($useLimit) $sql .= " LIMIT :limit OFFSET :offset";
