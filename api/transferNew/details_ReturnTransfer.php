...
try {
    ...
    if (!$equipment_id) {
        echo json_encode(["status" => "error", "message" => "equipment_id is required"]);
        exit;
    }

    // ดึงข้อมูลเครื่องมือและการโอนย้าย
    $sql = "
        SELECT 
          e.equipment_id,
          e.asset_code,
    ...
    if (!$row) {
        echo json_encode([
            "status" => "error", 
            "message" => "ไม่พบข้อมูลเครื่องมือ"
        ], JSON_UNESCAPED_UNICODE);
        exit;
    }

    // ดึงข้อมูลผู้ดูแลหลักจาก subcategory_id
    ...
    if ($subcategory_id) {
        $adminSql = "
            ...
            ORDER BY gu.group_user_id, u_admin.full_name
        ";
        ...
        foreach ($admins as $admin) {
            ...
            if (!isset($adminGroupsTemp[$groupId])) {
                $adminGroupsTemp[$groupId] = [
            ...
            if ($admin['full_name']) {
                $adminGroupsTemp[$groupId]['user_group'][] = [
                    'ID' => (int) $admin['ID'],
        ...
        $adminGroups = array_values($adminGroupsTemp);
    }
    ...
    } elseif ($row['transfer_type'] === "โอนย้ายชั่วคราว" && $row['status'] == 1) {
        $status_text = "คืนแล้ว";
    } else {
        $status_text = "ไม่มีข้อมูลการโอน";
    }
    ...
    $equipmentData = [
        ...
        'reason' => $row['reason'],
        'admins' => $adminGroups,
        'transfer_user' => $row['transfer_user_name'] ? [
        ...
        'recipient_user' => $row['recipient_user_name'] ? [
