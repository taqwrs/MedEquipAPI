...
if (isset($input['data']) && is_array($input['data'])) {
    ...
    try {
        ...
        if ($r_id !== null && $r_id !== 9 && $r_id !== 6) {
            $updateQuery .= " AND `ward` = :user_department";
        }
        ...
        foreach ($data as $index => $row) {
            ...
            if ($r_id !== null && $r_id !== 9 && $r_id !== 6) {
                if ($ward !== $dep) {
                    $errors[] = "Row $index: No permission to update ward '$ward'. You can only update ward '$dep'";
                    continue;
                }
            }
            ...
            try {
                ...
                if ($success && $updateStmt->rowCount() > 0) {
                    $affected++;
                } else if ($success && $updateStmt->rowCount() === 0) {
                    $errors[] = "Row $index: No matching record found or no permission to update";
                }
        ...
        $response = [
            "status" => "success", 
            "message" => "Updated $processed rows, affected $affected rows.",
            "details" => [
                "processed" => $processed,
                "affected" => $affected,
                "errors" => $errors,
